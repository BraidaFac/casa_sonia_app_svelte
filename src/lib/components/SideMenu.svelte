...
<div
	class={`fixed  bg-gray-800 left-0 top-0 md:min-h-64 md:w-1/3  w-full h-full  rounded-md shadow-md  z-50 transform transition-transform ${$sideBarStatus ? 'translate-x-0' : '-translate-x-full'} overflow-auto`}
>
	<div
		class="side-menu__header flex justify-between b p-3 items-center h-16"
	...
	<div class="side-menu__body p-3 rounded-lg py-5 h-full">
		...
		<ul class="side-menu__list h-full">
					...
					<a
						...
						onclick={(e) => {
							...
							if (contentToShow.contentType === 'gsr') {
								...
								contentToShow.content = gsr;
								contentToShow.contentType = 'gsr';
								searchStore.setSearch(item.descripcion);
								$sideBarStatus = false;
							} else {
								contentToShow.content = gsr;
								contentToShow.contentType = 'gsr';
